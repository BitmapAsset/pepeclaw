...
description: >
  Autonomous project health monitoring, auto-triage, and decision engine for
  multi-project environments. Scores project health, tracks velocity, maps
  dependencies, and generates actionable briefings.
  Triggers on: "project health", "war room", "project status", "triage projects",
  "blocker report", "dependency map".
metadata:
  openclaw:
    triggers:
      - "project health"
...
### Score a project's health
```bash
./scripts/health-scorer.sh /path/to/project
./scripts/health-scorer.sh /path/to/project --format json
```

### Track velocity
```bash
./scripts/velocity-tracker.sh /path/to/project
./scripts/velocity-tracker.sh /path/to/project --weeks 12 --format json
```

### Run auto-triage
```bash
./scripts/auto-triage.sh /path/to/project
./scripts/auto-triage.sh /path/to/project --threshold 60
```

### Map dependencies
```bash
./scripts/dependency-mapper.sh /path/to/workspace
./scripts/dependency-mapper.sh /path/to/workspace --format dot
```

## Data Storage

All data lives in workspace files — no external database needed:

| File | Format | Purpose |
|------|--------|---------|
| `/projects/{name}/STATE.md` | Markdown | Per-project health state |
| `references/project-state-schema.json` | JSON Schema | Project state structure |
| `references/health-score-spec.md` | Markdown | Scoring rubric |
...
- `TELEGRAM_CHAT_ID` — Target chat or group ID for notifications
- `WAR_ROOM_THRESHOLD` — Health score threshold for auto-triage (default: 50)

## Cron Setup

```
# Health scoring every 6 hours
0 */6 * * * /path/to/skills/project-war-room/scripts/health-scorer.sh <project-path>

# Dependency mapping daily at midnight
0 0 * * * /path/to/skills/project-war-room/scripts/dependency-mapper.sh <workspace-path>
```
...
├── SKILL.md                          # This file
├── scripts/
│   ├── health-scorer.sh              # Composite health score (0-100)
│   ├── velocity-tracker.sh           # Velocity metrics and trends
│   ├── auto-triage.sh                # Auto-triage on low scores
│   └── dependency-mapper.sh          # Cross-project dependency graph
├── references/
│   ├── health-score-spec.md          # Full scoring rubric
│   └── project-state-schema.json     # JSON schema for project state
└── data/
    └── (project STATE.md files)      # Per-project state
```
