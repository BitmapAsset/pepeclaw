...
description: >
  Time-value optimization for task scheduling and prioritization. Treats time as a
  market commodity — scoring urgency, batching low-priority work, detecting
  procrastination, and calculating opportunity costs.
  Triggers on: "what should I work on", "prioritize tasks", "batch tasks",
  "procrastination", "opportunity cost".
metadata:
  openclaw:
    triggers:
...
| File | Format | Purpose |
|------|--------|---------|
| `data/tasks.json` | JSON array | Task definitions conforming to schema |

## Cron Setup

The temporal arbitrage engine is typically invoked on-demand rather than on a
fixed schedule. Integrate it with your task management workflow.
...
- **Morning Brief**: Top priority tasks surface in the daily brief
- **Project War Room**: Blocker resolution tasks get priority boost
- **Skill Genome**: Task completion patterns inform skill fitness
...
├── SKILL.md                               # This file
├── scripts/
│   ├── time-scorer.sh                     # Score task urgency (0-100)
│   ├── batch-detector.sh                  # Group tasks for batching
│   └── procrastination-tracker.sh         # Track deferrals and avoidance
├── references/
│   └── task-schema.json                   # JSON schema for task data
└── data/
